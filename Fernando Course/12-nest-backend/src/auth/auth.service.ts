...
export class AuthService {
  ...
  async login( loginDto : LoginDto ): Promise<LoginResponse> {
    ...
    if( !bcryptjs.compareSync( password, user.password )) throw new UnauthorizedException(`Not valid credentials - wrong password ${ email }`);

    const { password:_, ...restInfo } = user.toJSON();
